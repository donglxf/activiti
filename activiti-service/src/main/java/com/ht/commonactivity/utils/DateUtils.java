...
//		return format;
     ...
     * @param date1 * 参照日期 *
     * @param date2 * 比较日期 *
     * @return 参照日期与比较日期之间的分钟差异，正数表示参照日期在比较日期之后，0表示两个日期同一分钟，负数表示参照日期在比较日期之前
     */
...
//		}
     ...
     * @return
     */
    public static String[] minusDay(Date startDate, Date endDate, String pattern) {
        if (StringUtils.isEmpty(pattern)) {
            pattern = OYYYY_MM_DD;
        }
        int days = minusDays(startDate, endDate);
        String[] str = new String[days];
        for (int i = 0; i < str.length; i++) {
            Date tem = dayOffset(endDate, -i);
            str[i] = format(tem, pattern);
        }
        return str;
    }
    ...
    public static void main(String[] arg){
        ...
        Date beforeDate = dayOffset(currentDate, -10);

        System.out.println(format(currentDate, OYYYY_MM_DD));
        System.out.println(format(beforeDate, OYYYY_MM_DD));
        String[] a=minusDay(beforeDate,currentDate,"");
